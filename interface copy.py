...
# cindy
class Client():
  def __init__(self, uid, replica_group_id, queue, bsz=32, epochs=10, indices=[0, 50], byzantine=False):
  ...
  def train(self, round_num):
    ...
    # print("train check parameters grad: ", list(self.model.parameters())[0].requires_grad)
  ...
  def send_message(self, msg):
    ...
    #   msg_dict_np[k] = torch.clone(v.detach())
    # msg.content = msg_dict_np
    self.queue.put(msg)
...
class Server: #todo: next level of byzantine is wrong #s and use replicas for agreement
  ...
  #     for k,v in valid_messages[i].content.items():
  #       if not(k in avg_messages):
  #         avg_messages[k] = (weights[i] * v)#torch.clone(v.detach()))
  #       else:
  #         avg_messages[k] += (weights[i] * v)#torch.clone(v.detach()))
  #       # valid_messages[0].content[k] += (weights[i] * v)
  #   for k,v in avg_messages.items():
  #     avg_messages[k] = avg_messages[k] / len(valid_messages)
  #   return (avg_messages, bad_client_ids)
  ...
  def find_deviation(self, all_state_dicts, averages):
    ...
    for (client_id, model) in all_state_dicts:
      ...
      for k, param in model.items():
        raw_param_deviation = averages[k] - param
        std_param_deviation = torch.mul(raw_param_deviation, raw_param_deviation)/averages[k]
        # synthesized_param_deviation = torch.sum(torch.mul(raw_param_deviation, raw_param_deviation))/torch.numel(param)
        synthesized_param_deviation = torch.sum(std_param_deviation)
  ...
  def deviations_to_byzantine(self, client_deviations, round_num, total_rounds):
    ...
    print("printing client deviations: ")
    for i in range(len(ids)):
      print (ids[i], ": ", deviations[i])
    ...
    curr_threshold = ((max_threshold - min_threshold) * (1 - round_num/total_rounds)) + min_threshold
    print("curr threshold: ", curr_threshold)


    #threshold_problem_params = 7
    # threshold_problem_params = 10 * (1 - round_num)  #min=num_params*0.3 and max=num_params*0.9
    byz_client_idxs = np.where(byz_client_idxs_per_param > curr_threshold)[0]
...
class RunTraining:
  ...
  def forward(self):
    ...
    for round_num in range(self.num_rounds): #num global rounds
      ...
      print("detected bad client ids: ", bad_client_ids)
...
def main():
  runner = RunTraining(num_clients=10, num_replicas=1, num_rounds=5, num_byzantine=0) #comment
  runner.forward()
...
# parameters: timeout threshold, harshness of outlier detection